...
Installing an **R** package as we just did, only needs to be done once. Yet each time you want to use the functionality of the **R** package in your current **R** session, you need to activate it. You can do this via the following command:
```{r, eval=FALSE}
library(this.path)
```
...
Both commands above to the exact same: they open the help file of the function you specified. An **R** Help page will only be opened if the function exists!

You will explore **R** Help pages in more detail during later sessions!
...
**R** should tell you that no documentation for the requested function exists. 
...
Such a broader search is useful if you are not sure about the exact name of a function.
