...
You can execute a specific line
...
**R** will ask you 'Do you want to install from sources the package which needs compilation?' Choose No (TO BE DISCUSSED IF INCLUDED IN SESSION 1!)

## Loading R package

While installing an **R** package only needs to be done once, each time you want to use the functionality of the **R** package, you need to load it into your **R** session. You can do this via the following command:
```{r, eval=FALSE}
library(this.path)
```

Now **R** has access to the functionality that the **this.path** offers.

The **this.path** package offers a convenient alternative to setting your working directory via the Menu bar. You can simply use the following command to accomplish exactly the same:
```{r, eval=FALSE}
setwd(dirname(this.path::this.path()))
```
It may be convenient to enter this at the top of your **R** script to ensure that you are always working in the directory where your **R** Script is located.
