...
# negative indexing
data2 <- data_cli[, -c(1:2, 4:13, 15:18, 20:ncol(data_cli))][, c(2, 1, 3)]


# check
identical(data, data1) & identical(data1, data2) 
```
...
Sometimes we want to transform data or add information. For instance, we could be interested in adding a variable `season`, with values `spring`, `summer`, `autumn`, `winter` to our climate data set.

To add a new variable to a data.frame, use the `$` operator
```{r list10, eval = TRUE}
...
get_season <- function(date) {
  ...
  if (m %in% 6:8) return("summer")
  if (m %in% 9:11) return("autumn")
  return("winter")
}
get_season <- Vectorize(get_season)
data_cli$season <- get_season(as.Date(data_cli$YEARMODA))

head(data_cli[, c("YEARMODA", "season")], 3)
```
...
head(data_cli[, c("YEARMODA", "season", "NAME", "TEMP", "temp_k")], 3)
...
```{r list13, eval = TRUE}
# split the data set
data <- data_cli[data_cli$NAME != "MAASTRICHT" ,]
data_M <- data_cli[data_cli$NAME == "MAASTRICHT" ,]

# join the two data sets 
data_new <- rbind(data, data_M)
identical(data_cli, data_new)

# preserve order
identical(data_cli, data_new[order(data_new$X), ])
```
...
If we want to generate a new data.framem, we can collect the relevat information via the `data.frame()` function: 
```{r list15, eval = TRUE}
# split the data set
...
# stepAIC(lm(MAASTRICHT ~ trend, data = data),

plot(data$temp, type = "l")
lines(fitted(final), col="red")
...
summary(final)
plot(data$temp, type = "l")
lines(fitted(final), col="red", lwd = 3)
```
